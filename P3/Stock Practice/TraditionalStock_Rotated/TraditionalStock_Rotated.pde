...
void singleStock(int j) {
    noStroke();
    fill(color(initialColor[0] + colorStep[0]*j,initialColor[1] + colorStep[1]*j,initialColor[2] + colorStep[2]*j, 20));
    rect(height*j/totalLength, width*(1 - priceListHigh[j]/graphMax)-10, 1, (priceListHigh[j] - priceListLow[j])/graphMax*width);
}
